...
import { Search, MessageCircle, Shield, CheckCircle, Truck, Star, Eye, ShoppingCart, Users, Headphones, Bike, DrillIcon as Drone, Battery, Monitor, Camera, X, ChevronLeft, ChevronRight, Menu, Clock, Package } from 'lucide-react'
...
type HeroProduct = {
  ...
  image: string
  seller: string
  badge: "VERIFIED PRO" | "FOUNDING SELLER" | string
}
...
export default function MarketplaceLanding() {
  ...
  const nextSlide = () => {
    setCurrentSlide((prev) => (heroProducts.length ? (prev + 1) % heroProducts.length : 0))
  }

  const prevSlide = () => {
    setCurrentSlide((prev) => (heroProducts.length ? (prev - 1 + heroProducts.length) % heroProducts.length : 0))
  }

  // Auto-rotate every 5s, pause on hover
  useEffect(() => {
    if (!heroProducts.length || isCarouselHovered) return
    const id = setInterval(() => {
      setCurrentSlide((prev) => (prev + 1) % heroProducts.length)
    }, 5000)
    return () => clearInterval(id)
  }, [heroProducts.length, isCarouselHovered])
                ...
                <div className="space-y-6">
                  ...
                  <div className="space-y-4">
                    <p className="text-gray-300 italic">
                      {"After being deplatformed from major marketplaces, I helped establish ArbVault's seller standards. Every piece of equipment I sell is authenticated and ships same day."}
                    </p>
          ...
          <div className="grid md:grid-cols-2 gap-12 mb-12">
            <Card className="bg-gray-900 border-red-500/30">
              <CardContent className="p-6">
                ...
                <p className="text-gray-300 mb-4">Pro equipment marketplaces are full of unverified sellers and risky transactions.</p>
                  ...
                  <div className="p-4 space-y-3">
                    <div>
                      <div className="flex justify-between items-start">
                        ...
                        <Badge
                          className={`${
                            product.sellerType === "Founding Seller"
                              ? "bg-amber-500/20 text-amber-400 border border-amber-500/30"
                              : "bg-green-500/20 text-green-400"
                          } text-xs`}
                        >
                    ...
                    <div className="flex items-center text-sm">
                      ...
                      {(product as any).lastUnit && <span className="ml-2 text-amber-500 text-xs">Last unit available</span>}
                      {!(product as any).lastUnit && (product as any).stock > 0 && (
                        <span className="ml-2 text-amber-500 text-xs">Only {(product as any).stock} left in stock</span>
                      )}
                    </div>
                    ...
                    <div className="flex space-x-2">
                      <Button size="sm" className="flex-1 bg-blue-600 hover:bg-blue-700" disabled={(product as any).soldOut}>
      ...
      <footer className="bg-gray-950 border-t border-gray-800">
        <div className="container mx-auto px-4 py-12">
          ...
          <div className="border-t border-gray-800 mt-12 pt-8 text-center text-gray-300">
            <p>© 2025 ArbVault. All rights reserved. Built for professionals, by professionals.</p>
          </div>
        </div>
      </footer>
        ...
        <div className="fixed bottom-24 left-6 z-40 animate-slide-in">
          <Card className="bg-gray-900 border-green-500 border shadow-lg">
            <CardContent className="p-3 flex items-center space-x-3">
              ...
              <p className="text-sm text-gray-300">🛡️ {toastProduct} shipped to {toastLocation}</p>
            </CardContent>
          </Card>
        </div>
      )}
